...
namespace Finbourne.Workflow.Sdk.Extensions
{
    ...
    public class ApiFactory : IApiFactory
    {
        ...
        public ApiFactory(ApiConfiguration apiConfiguration)
        {
            ...
            if (!Uri.TryCreate(apiConfiguration.WorkflowUrl, UriKind.Absolute, out var _))
            {
                if (string.IsNullOrWhiteSpace(apiConfiguration.WorkflowUrl))
                    throw new ArgumentNullException(
                        nameof(apiConfiguration.WorkflowUrl),
                        $"Workflow Uri missing. Please specify either FBN_WORKFLOW_API_URL environment variable or workflowUrl in secrets.json.");

                throw new UriFormatException($"Invalid Uri: {apiConfiguration.WorkflowUrl}");
            }

            // Create configuration
            var configuration = new TokenProviderConfiguration(tokenProvider)
            {
                BasePath = apiConfiguration.WorkflowUrl,
            };
